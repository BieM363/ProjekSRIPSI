{{-- resources/views/home/menu/realisasi-kinerja-edit.blade.php --}}
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Ramayana - Edit Realisasi Kinerja</title>
        ...
        <div class="container-fluid">
            ...
            <div class="card bg-purple-50 shadow-md">
                <div class="card-header bg-purple-700 text-white">
                    <h3 class="text-xl font-bold m-0">Edit Realisasi Kinerja</h3>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('realisasi_kinerja.update', $realisasi->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="mb-4">
                                ...
                                <select name="pegawai_id" class="w-full p-2 border border-purple-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                    @foreach($pegawai as $p)
                                        <option value="{{ $p->id }}" 
                                            {{ $realisasi->pegawai_id == $p->id || old('pegawai_id') == $p->id ? 'selected' : '' }}>
                                            {{ $p->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="indikator_id" class="w-full p-2 border border-purple-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                    @foreach($indikator as $i)
                                        <option value="{{ $i->id }}" 
                                            {{ $realisasi->indikator_id == $i->id || old('indikator_id') == $i->id ? 'selected' : '' }}>
                                            {{ $i->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="periode" class="w-full p-2 border border-purple-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                    @foreach($periodes as $p)
                                        <option value="{{ $p }}" 
                                            {{ $realisasi->periode == $p || old('periode') == $p ? 'selected' : '' }}>
                                            {{ $p }}
                                        </option>
                                    @endforeach
                                </select>
